...
// Manager implements the PackageCacheManager interface
type Manager struct {
	cacheDir    string
	indexPath   string
...
func (m *Manager) migrateOldIndex(data []byte) error {
...
func (m *Manager) findPackage(name, version, arch string) (*PackageInfo, int) {
...
func (m *Manager) addPackage(packageInfo PackageInfo) {
	m.index.Packages = append(m.index.Packages, packageInfo)
}

// updatePackage updates an existing package in the index
func (m *Manager) updatePackage(index int, packageInfo PackageInfo) {
...
func (m *Manager) removePackage(index int) {
	if index >= 0 && index < len(m.index.Packages) {
...
func (m *Manager) EnsurePackage(ctx context.Context, packageName, version, arch string, sshRunner interfaces.SSHCommandRunner, nodeName string) error {
...
func (m *Manager) UploadPackageWithSCP(ctx context.Context, packageName, version, arch string, sshRunner interfaces.SSHRunner, nodeName string) error {
...
func (m *Manager) EnsurePackageWithSCP(ctx context.Context, packageName, version, arch string, sshRunner interfaces.SSHRunner, nodeName string) error {
...
func (m *Manager) GetLocalPackagePath(packageName, version, arch string) (string, error) {
	packageInfo, _ := m.findPackage(packageName, version, arch)
	if packageInfo == nil {
		key := PackageKey(packageName, version, arch)
		return "", fmt.Errorf("package %s not found in cache", key)
...
func (m *Manager) IsPackageCached(packageName, version, arch string) bool {
...
func (m *Manager) downloadAndCachePackage(ctx context.Context, packageName, version, arch string) error {
...
func (m *Manager) UploadPackageToNode(ctx context.Context, packageName, version, arch string, sshRunner interfaces.SSHCommandRunner, nodeName string) error {
	...
	if _, err := sshRunner.RunSSHCommand(nodeName, createDirCmd); err != nil {
		return fmt.Errorf("failed to create remote directory: %w", err)
	}

	// Check if package already exists on remote node
	checkCmd := fmt.Sprintf("test -f %s && echo 'exists' || echo 'not_exists'", packageInfo.GetRemotePath())
	output, err := sshRunner.RunSSHCommand(nodeName, checkCmd)
	if err != nil {
		return fmt.Errorf("failed to check if package exists on remote node: %w", err)
	}
	...
	if fileTransfer, ok := sshRunner.(interfaces.SSHRunner); ok {
		...
		if err := fileTransfer.TransferFile(nodeName, packageInfo.LocalPath, packageInfo.GetRemotePath()); err != nil {
...
func (m *Manager) uploadFileViaSSH(localPath, remotePath string, sshRunner interfaces.SSHCommandRunner, nodeName string) error {
	// Read local file
	...
	for i := 0; i < totalChunks; i++ {
		...
		if _, err := sshRunner.RunSSHCommand(nodeName, cmd); err != nil {
...
func (m *Manager) CleanupOldPackages(maxAge time.Duration) error {
...
func (m *Manager) ListCachedPackages() []PackageInfo {
...
func (m *Manager) GetCacheStats() (int, int64, int64) {
